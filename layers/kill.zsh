...
spacezsh.kill.widget() {
  local fzf_expect='enter,ctrl-k,alt-k,ctrl-x,alt-x,ctrl-l,alt-l'
  local fzf_header=$'\nShortcuts: enter: select PID,\tctrl-k: kill,\talt-k: sudo kill,\tctrl-x: kill -9,\talt-x: sudo kill -9,\tctrl-l: lsof,\talt-l: sudo lsof'
  local FZF_HEIGHT=$([[ -n "$FZF_TMUX" && -n "$TMUX_PANE" ]] && echo ${FZF_TMUX_HEIGHT:-40%} || echo 100%)
